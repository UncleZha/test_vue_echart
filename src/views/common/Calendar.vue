<template>
  <div class="times"
    ...
    <div class="row">
      ...
      <div class="todayBtn"
           @click="backToday()">返回今天</div>
    </div>
    <div class="time-bottom">
      <p class="week">
        <span v-for="(item, index) of weeks"
              :key="index">{{ item }}</span>
      </p>
      <div class="dayList"
           ref="dayList"
        ...
        <van-swipe class="my-swipe"
          ...
          <van-swipe-item>
            <div class="beforeday"
                 ref="beforeday">
              <span v-for="(item, index) of beforemonths"
                    :key="index"
                    @click="getIndex(index, item.date)"><span class="dayitem"
                      :class="{
                    fontcolor: item.state,
                    finish: item.date == flag,
                    test: !item.state,
                  }"
                      tapmode="">{{ item | today }}</span>
                <span class="rest"
                      v-if="item.leaveStatus">{{item.leaveStatus && item.type|statusName}}</span>
                <span class="rest"
                      v-if="item.unfinishedStatus">{{
                  item.unfinishedStatus ? "x" : ""
                }}</span>
                <span class="rest"
                      v-if="!item.unfinishedStatus && !item.leaveStatus"></span></span>
            </div>
          </van-swipe-item>
          <van-swipe-item>
            <div class="day"
                 ref="day">
              <span v-for="(item, index) of months"
                    :key="index"
                    @click="getIndex(index, item.date)"><span class="dayitem"
                      :class="{
                    fontcolor: item.state,
                ...
                <span class="rest"
                      v-if="!item.unfinishedStatus && !item.leaveStatus"></span>
              </span>
            </div>
          </van-swipe-item>
          <van-swipe-item>
            <div class="afterday"
                 ref="afterday">
              <span v-for="(item, index) of aftermonths"
                    :key="index"
                    @click="getIndex(index, item.date)"><span class="dayitem"
                      :class="{
                    fontcolor: item.state,
